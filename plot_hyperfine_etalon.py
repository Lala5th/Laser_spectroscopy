...
ax1.axvspan(etalon_mod['t'][peaks[0]],etalon_mod['t'][peaks[-1]],alpha=0.25,hatch='/',color='green',label='Etalon range')
ax2.axvspan(etalon_mod['t'][peaks[0]],etalon_mod['t'][peaks[-1]],alpha=0.25,hatch='/',color='green',label='Etalon range')
ax2.plot(modprobe_t,removed_bg,label='Transmitted intensity')
plt.xlabel("$\\nu$ [MHz]")
...
def fit_structure(xmin,xmax,m1=0,m2=0,m3=0,p0=None,plot=True):
    ...
    if(plot):
        ...
        a1.plot(modprobe_t[lim],modprobe[lim],label='Transmitted intensity')
        a1.plot(modprobe_t[lim],bg(modprobe_t,*bgopt)[lim],label='Fitted background')
        a2.plot(modprobe_t[lim],removed_bg[lim],label='Background removed intensity')
        a2.plot(modprobe_t[lim],structure(modprobe_t,*fit)[lim],'r',label='Fitted spectrum')
        res = removed_bg-structure(modprobe_t,*fit)
        a3.plot(modprobe_t[lim],res[lim],label = 'Residuals')
        a1.grid()
        a2.grid()
        a3.grid()
        a1.legend()
        ...
        a3.legend()
        f2.tight_layout()
        f2.align_ylabels([a1,a2,a3])
    return (fit, cov)

def fit_total(amin,amax,bmin,bmax,am1,am2,am3,bm1,bm2,bm3,comp_plot=True):
    assert(am1<am2)
    assert(am2<am3)
    assert(bm1<bm2)
    assert(bm2<bm3)
    ...
    afit, acov = fit_structure(amin,amax,am1,am2,am3,plot=not comp_plot)
    print("Fit B parameters:")
    bfit, bcov = fit_structure(bmin,bmax,bm1,bm2,bm3,plot=not comp_plot)
    print("\nS1/2 Splitting:")
    ...
    if(comp_plot):
        ...
        f2.show()
        a11.plot(modprobe_t[alim],modprobe[alim],label='Transmitted intensity')
        a11.plot(modprobe_t[alim],bg(modprobe_t,*bgopt)[alim],label='Fitted background')
        a12.plot(modprobe_t[blim],modprobe[blim],label='Transmitted intensity')
        a12.plot(modprobe_t[blim],bg(modprobe_t,*bgopt)[blim],label='Fitted background')
        a21.plot(modprobe_t[alim],removed_bg[alim],label='Background removed intensity')
        a21.plot(modprobe_t[alim],structure(modprobe_t,*afit)[alim],'r',label='Fitted spectrum')
        a22.plot(modprobe_t[blim],removed_bg[blim],label='Background removed intensity')
        a22.plot(modprobe_t[blim],structure(modprobe_t,*bfit)[blim],'r',label='Fitted spectrum')
        res1 = removed_bg-structure(modprobe_t,*afit)
        a31.plot(modprobe_t[alim],res1[alim],label = 'Residuals')
        res2 = removed_bg-structure(modprobe_t,*bfit)
        a32.plot(modprobe_t[blim],res2[blim],label = 'Residuals')
        a11.legend()
        a12.legend()
        a21.legend()
        a22.legend()
        ...
        a32.legend()
        f2.tight_layout()
        f2.align_ylabels([a11,a21,a31])
        f2.align_ylabels([a12,a22,a32])
